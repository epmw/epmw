...
void swapEndianUniversal(uint8_t *buffer, const uint8_t op_size)
{
	uint8_t i;
	uint8_t temp;
	...
	{
		temp = buffer[i];
		buffer[i] = buffer[op_size - 1 - i];
		buffer[op_size - 1 - i] = temp;
	}
}
...
uint8_t bigAddVariableSizeNoModuloDiffOpSizes(uint8_t *r, uint8_t *op1, uint8_t op1_size, uint8_t *op2, uint8_t op2_size)
{
	uint16_t partial;
	uint8_t carry;
	uint8_t i;
	...
	carry = 0;
	uint8_t bigger_op_size = (op1_size > op2_size) ? op1_size : op2_size;
	for (i = 0; i < bigger_op_size; i++)
	{
		uint8_t op1_val = (i < op1_size) ? op1[i] : 0;
		uint8_t op2_val = (i < op2_size) ? op2[i] : 0;
		partial = (uint16_t)((uint16_t)op1_val + (uint16_t)op2_val + (uint16_t)carry);
		r[i] = (uint8_t)partial;
		carry = (uint8_t)(partial >> 8);
	}
	return carry;
...
void bigModuloTo256(BigNum256 r, uint8_t *op1, uint8_t op1_size)
{
	uint8_t temp[64];
	uint8_t full_r[64];
	memcpy(full_r, op1, op1_size);
	uint8_t remaining;
	...
	while (remaining > 32)
	{
		...
		memset(&(full_r[32]), 0, op1_size - 32);
